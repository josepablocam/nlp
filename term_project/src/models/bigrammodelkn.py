...
class BigrammodelKN(Bigrammodel):
    """ bigram-based POS tagger with kneser-ney smoothing for transition probs"""

    def __init__(self, labels, map_unk, unk_ct = 2, delta = 0.5): #default to 0.5 discounting
        Bigrammodel.__init__(self, labels, map_unk, unk_ct)
        self.delta = delta
      
    def train(self, corpus):
        Bigrammodel.train_emit(self, corpus) #emission probabilities continue be handeled the same way
        self.train_trans(corpus) #transition probabilities are smoothed with KN
